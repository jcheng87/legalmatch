...
def prepare_files(dir_path,vendor_file_name, onit_file_name):
    #VC Confirm file
    #['Vendor ID', 'Invoice Number]
    vc_df = pd.read_csv(os.path.join(dir_path,vendor_file_name),dtype={'Vendor ID':'object','Invoice Number':'object'}, index_col=['Vendor ID', 'Invoice Number'])
    vc_df = vc_df.sort_index(level=vc_df.index.names)
    
    #Onit file
    #['Vendor ID', 'Invoice Number']
    onit_df = pd.read_csv(os.path.join(dir_path,onit_file_name),dtype={'Vendor ID':'object','Invoice Number':'object'}, index_col=['Vendor ID', 'Invoice Number'])
    ...
    onit_check[onit_file_name] = True
    vc_check = vc_df.copy()
    vc_check[vendor_file_name] = True

    #merge onit check with vc_final and vc check with onit final
    vc_final = vc_final.merge(onit_check[onit_file_name], on = ['Vendor ID', 'Invoice Number'], how='left') #merge onit with confirm df
    onit_final = onit_final.merge(vc_check[vendor_file_name], on = ['Vendor ID', 'Invoice Number'], how='left') #merge vc with onit df

    return [vc_final, onit_final]
...
def legal_match():
    ...
    # vc_final, onit_final = 
    vc_final,onit_final = prepare_files(dir_path,CONFIRMS,ONIT)
    ...
    for file in ACMBILLS:
        ...
        file_df.set_index(['Vendor ID', 'Invoice Number'], inplace=True)
        file_df[file] = True

        #merge with final dfs
        vc_final = vc_final.merge(file_df[file], on=['Vendor ID','Invoice Number'], how='left')
        onit_final = onit_final.merge(file_df[file], on=['Vendor ID','Invoice Number'], how='left')
